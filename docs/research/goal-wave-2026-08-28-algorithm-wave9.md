...
- [x] `docs/research/p9_expanded_gage_unbalanced.md`（Primary URL + 公式 + 访问日期）

- [x] domain + Facts + AnalysisService + command（menu_path/menu_group）

- [x] 不平衡 GLM 方差分量；VarComp + %Contribution + %Study Var + NDC

- [x] interpretation + serialization + help

- [x] **独立**多页对话框（≥4 页）；≠ `expanded_gage_rr` 对话框
...
- [x] `docs/research/p9_split_plot_analyze.md`

- [x] 裂区双误差项；难改/易改因子；ANOVA + WP/SP residuals
...
- [x] `docs/research/p9_mixture_process_variable.md`

- [x] Scheffé + 1 过程变量 + 可选组分×过程交互

- [x] 完整竖切 + 诊断 + formula_reference 测试

- [x] UI 组分/过程变量/模型分页；≠ `mixture_analyze` 对话框
...
- [x] `docs/research/p9_manova_one_way.md`

- [x] 2～4 响应 + 1 因子；Wilks/Pillai/LH/Roy；H/E SSCP
...
- [x] `verify_algorithm_wave9_track.py` PASS
...
- [x] Expanded Gage 不平衡未并入 `expanded_gage_rr` 对话框

- [x] Mixture 过程变量未并入 `mixture_analyze` 对话框「加勾选」

- [x] Split-plot 未塞进 `doe_factorial` 单页
