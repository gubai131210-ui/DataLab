...
ENTRIES.append(entry(
    ...
    "用 Anderson–Darling 检验一列数据是否与正态分布一致。",
    "对标准化后的排序观测计算 A² 与 Stephens 修正 A²*，再分段近似 p 值。未拒绝不等于已证明正态。",
    "选择一个数值列。默认 α=0.05。",
    ["取有限值，排序 x_(1)≤…≤x_(n)。",
     "标准化 z_i=(x_(i)−x̄)/s，F_i=Φ(z_i)，对 F_i 做极小裁剪避免 ln(0)。",
     "计算 A² 与 A²*，查分段近似得到 p。",
     "比较 p 与 α，输出 reject / fail_to_reject / not_computed。"],
    [("A²", "Anderson–Darling 统计量"), ("A²*", "Stephens 样本量修正"),
     ("Φ", "标准正态 CDF"), ("α", "显著性水平，默认 0.05")],
    [fb("Anderson–Darling",
    ...
    "AD 表（A²、A²*、p、判定）+ 正态概率图 + 直方图。悬停点对应原始行。",
...
ENTRIES.append(entry(
    "outlier_test", "异常值检验（Grubbs）", "基础统计", "统计 > 基础统计",
    "outlier_test", "outlier_test", "grubbs_test",
    "检验单个最极端观测是否能被看作异常值。",
    "Grubbs 用离均值最远的点除以样本标准差。P 由 t_{n−2} 反解。检出不等于必须删除。",
    "一个数值列；置信水平默认 95%；备择 two_sided / less / greater。",
    ["取有限值，计算均值与 s。", "找使 |y−x̄| 最大的点（或单侧最大/最小）。",
     "G=max|y−x̄|/s，由 t 分布得到 p。", "与 α=1−置信水平 比较。"],
    [("G", "Grubbs 统计量"), ("s", "样本标准差"), ("t_{n−2}", "用于反解 p 的 t 分布")],
    [fb("Grubbs", "G = max|y−x̄| / s ；双侧 p 由 t_{n−2} 反解。",
        [t("G = "), frac([abs_n("y − x̄"), t(" 的最大值")], "s")],
        "只检验最极端的一个点。", "n≥3 且 s>0。")],
    ["p<α 提示最极端点与其余点不一致。", "不得据此自动删点或写已确认异常。"],
    "n<3、s=0 或未选列时不计算。",
    "G、p 与个体值图，异常候选点可识别。",
...
ENTRIES.append(entry(
    ...
    "度量两列数值的线性（Pearson）或单调（Spearman）相关。",
    "每一对变量用 pairwise 有效观测。H0: ρ=0。Pearson 区间用 Fisher z 变换。",
    "至少两列数值。方法 pearson（默认）或 spearman。置信水平默认 95%。",
    ["对每一对列取同时有效的行。", "Pearson 直接用原值；Spearman 先转平均秩再套 Pearson。",
     "计算 r、t、DF、p 和 Fisher z 区间。"],
    [("r", "相关系数，∈[−1,1]"), ("z", "atanh(r)"), ("SE(z)", "1/√(n−3)")],
    ...
    ["p 小只说明与 0 相关的证据强，不是因果。", "常量列或配对数不足则诊断。"],
    "少于两列、有效配对数不足、任一列为常量或置信水平非法时返回诊断。",
    "相关矩阵/表 + 散点或矩阵图。",
    "同时看 r 的大小、区间是否含 0 和散点形态。",
    "Spearman 用平均秩处理并列。",
    ["corr"],
))
...
ENTRIES.append(entry(
    ...
    "比较两组事件比例之差。",
    "每组可多行求和。差值 Δ=p̂1−p̂2；检验 Z 始终用分位 unpooled Wald。method=normal（默认）时 CI 为 Wald；method=wilson 时 CI 为 Newcombe–Wilson（两单比例 Wilson 端点再组合）。适用时另给 Fisher 精确检验。",
    "两组各自的事件列与试验列；可选 method=normal|wilson。",
    ["各组求和得到 x1,n1,x2,n2。", "p̂i=xi/ni。", "Δ 与 SE_sep 得 Z/P；按 method 输出 Wald 或 Newcombe–Wilson 区间；必要时 Fisher。"],
    [("Δ", "p̂1−p̂2"), ("SE_sep", "√[p̂1(1−p̂1)/n1+p̂2(1−p̂2)/n2]"),
     ("ci_method", "wald 或 newcombe_wilson")],
    ...
    ["一组缺失不拿另一组去填。", "解释是差值证据不是规格判定。", "不做 Agresti–Coull 或 Blaker。"],
...
ENTRIES.append(entry(
    ...
    "检验两组均值比值 ρ=μ_test/μ_ref 是否落入比值等价界限（非对数 TOST）。",
    "对 ρ 做两个单侧 t：t(δ)=(ȳ1−δ·ȳ2)/SE(δ)。输出 `ci_method=tost_ratio_1_minus_alpha` 的 100(1−2α)% Fieller 比值区间，以及 `both_pvalues_below_alpha`。`EquivalenceFacts.difference` 存 ρ̂。",
    "检验列、参考列、比值下/上界限（须 >0）、Welch 或 pooled、置信水平。",
    ["计算两组均值与 SD。", "ρ̂=ȳ_test/ȳ_ref。", "两侧 t 与 Fieller CI。"],
    [("ρ̂", "ȳ1/ȳ2"), ("δ1,δ2", "比值等价界限"), ("ci_method", "tost_ratio_1_minus_alpha")],
    [fb("均值比 TOST", "t1=(ȳ1−δ1ȳ2)/SE(δ1)，t2=(ȳ1−δ2ȳ2)/SE(δ2)。within_limits 当且仅当两单侧 p≤α。")],
    ["不得写成已证明等价。", "参考均值必须为正。", "本命令不做对数变换。"],
    "参考均值≤0、界限非法、Fieller 条件失败或方差不可用时诊断。",
    "描述统计、TOST 表、比值区间图。",
    "先确认界限是比值尺度（如 0.8～1.25），再看区间是否完整落入。",
