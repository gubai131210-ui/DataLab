...
for cid, used, notf, n, ds in [
    ("descriptive", "汇总测量值中心、散布与形状，可按产线/模腔分组。", "不能判断稳定性或规格符合性。", "n≥30；分组每组≥5", None),
    ("normality_test", "评估数据与正态假设偏离，为参数/非参数选型提供证据。", "未拒绝≠证明正态；小样本功效低。", "n≥20", None),
    ("outlier_test", "识别可能来自特殊原因的极端观测。", "统计离群≠必须删除；需工程确认。", "n≥20", None),
    ("correlation", "量化两列或多列数值的线性/单调关联强度。", "相关≠因果；不能预测规格合格。", "n≥30", None),
    ("one_sample_t", "检验样本均值是否偏离假设目标值。", "不能证明过程长期受控。", "n≥15", None),
    ("one_sample_z", "已知 σ 时检验均值；或大样本近似。", "σ 估计错误则结论无效。", "n≥30", None),
    ("two_sample_t", "比较两独立组均值差异。", "组间非独立时不可用；不等方差选 Welch。", "每组 n≥15", None),
    ("paired_t", "比较同一单元处理前后均值差。", "配对必须一一对应。", "配对 n≥15", None),
    ("one_proportion", "估计/检验单一事件比例。", "不能代表多阶段过程整体不良。", "事件数≥5", None),
    ("two_proportions", "比较两组比例差异。", "样本独立；小计数用精确法。", "每组 n≥30", None),
    ("one_poisson_rate", "估计单位长度/面积缺陷发生率。", "需明确观测长度/面积。", "缺陷总数≥10", None),
    ("two_poisson_rate", "比较两组 Poisson 发生率。", "长度单位需一致。", "每组缺陷≥5", None),
    ("variance_test", "比较两组或多样本方差是否齐性。", "对非正态敏感；小样本功效低。", "每组 n≥10", None),
    ("one_way_anova", "比较≥3 组均值是否全等。", "显著后需事后比较；残差需检查。", "每水平 n≥5", None),
    ("two_factor_anova", "评估两因子主效应与交互。", "不能外推未试验水平。", "每单元≥3 重复", None),
    ("regression", "建立预测因子与连续响应的关系。", "外推风险；残差诊断必备。", "n≥10×参数", None),
    ("logistic_regression", "建模二分类响应与因子关系。", "不能证明因果；需足够事件数。", "事件≥10", None),
    ("chi_square", "检验分类变量独立性。", "期望频数过小需合并类别。", "总 n≥50", None),
    ("cross_tabulation", "列联表汇总与比例展示。", "仅汇总不检验时需另做卡方。", "视列联表大小", None),
    ("chi_square_gof", "检验分类分布是否符合理论比例。", "不能证明数据生成机制。", "每类期望≥5", None),
    ("mann_whitney", "两独立组非参数位置比较。", "检验分布位置而非仅中位数（一般解释）。", "每组 n≥10", None),
    ("wilcoxon_signed_rank", "配对非参数差值检验。", "差值分布需近似对称。", "n≥15", None),
    ("sign_test", "配对符号检验，最稳健。", "功效低于 Wilcoxon。", "n≥15", None),
    ("kruskal_wallis", "多组非参数位置比较。", "显著后需事后非参数比较。", "每组 n≥5", None),
    ("friedman", "重复测量/区组非参数。", "区组内配对完整。", "区组≥10", None),
    ("mood_median", "多组中位数差异检验。", "对离群较稳健。", "每组 n≥10", None),
    ("cochran_q", "相关样本多组二分类比例。", "仅适用二元响应。", "k≥3 处理", None),
    ("mcnemar", "配对二分类变化检验。", "仅 2×2 配对表。", "discordant≥10", None),
    ("fisher_exact", "小样本 2×2 精确检验。", "大表计算慢。", "小计数", None),
    ("runs_test", "检验序列随机性。", "不能定位特殊原因来源。", "n≥25", None),
    ("anom", "识别哪些组均值偏离总均值（ANOM）。", "需正态/方差假设；不等 n 谨慎。", "每组 n≥5", None),
    ("anom_attribute", "属性数据 ANOM。", "小计数需合并。", "每组 n≥20", None),
    ("poisson_gof", "检验计数是否符合 Poisson。", "过度离散需负二项等。", "总计数≥20", None),
]:
...
for cid in ["one_sample_equivalence", "two_sample_equivalence", "two_sample_equivalence_ratio",
    ...
    add(cid,
        ...
        dataset_hint=None if "paired" in cid else None,
        common_mistakes=["等价限事后挑选。", "忽略方差估计方法。"],
        sources=["nist_eda", "montgomery_spc"])

# Control charts
CC = [
    ("imr", "单件慢节拍个体值与移动极差监控。", None),
    ("xbar_r", "子组 2–10 的均值与极差监控。", None),
    ("xbar_s", "子组较大时用标准差监控散布。", None),
    ("imr_rs", "I-MR-R/S 组合监控。", None),
    ("p_chart", "不合格品率监控（变样本量）。", None),
    ("np_chart", "不合格品数监控（固定样本量）。", None),
    ("c_chart", "固定检验面积缺陷数监控。", None),
    ("u_chart", "单位缺陷数监控（变检验单位）。", None),
    ("laney_p_chart", "过度离散时的 Laney P 图。", None),
    ("laney_u_chart", "过度离散时的 Laney U 图。", None),
    ("ewma", "对微小漂移敏感的 EWMA。", None),
    ("cusum", "累积和检测小偏移。", None),
    ("zone_chart", "区域规则辅助识别模式。", None),
    ("z_mr", "标准化个体与 MR 监控。", None),
    ("moving_average", "移动平均平滑监控。", None),
    ("g_chart", "几何分布间隔/良率事件监控。", None),
    ("t_chart", "时间间隔 T 图。", None),
    ("hotelling_t2", "多变量 T² 监控。", None),
    ("mewma", "多变量 EWMA。", None),
    ("generalized_variance", "多变量广义方差监控。", None),
]
...
# Capability / MSA
CAP = [
    ("capability", "估计 Cp/Cpk 等相对规格的能力指数。", None),
    ("nonnormal_capability", "非正态数据能力（如 Johnson/百分位）。", None),
    ("between_within_capability", "分解组间/组内变异的能力。", None),
    ("batch_capability", "批次间能力评估。", None),
    ("nonparametric_capability", "非参数百分位能力。", None),
    ("binomial_capability", "二项不良率能力。", None),
    ("poisson_capability", "Poisson 缺陷率能力。", None),
    ("capability_sixpack", "能力六合一图形诊断。", None),
    ("box_cox", "Box-Cox 变换辅助正态化。", None),
    ("distribution_identification", "拟合候选分布识别。", None),
    ("tolerance_intervals", "估计覆盖比例的总体区间。", None),
]
...
MSA = [
    ("gage_rr", "交叉量具 R&R。", None),
    ("emp_crossed", "展开交叉 Gage R&R。", None),
    ("expanded_gage_rr", "展开型 Gage R&R。", None),
    ("expanded_gage_unbalanced", "非平衡展开 Gage。", None),
    ("nested_gage_rr", "嵌套 Gage R&R。", None),
    ("msa_type1", "Type 1 偏倚与重复性。", None),
    ("attribute_agreement", "属性一致性/agreement。", None),
]
for cid, _, ds in MSA:
    add(cid,
...
GRAPH_DS = {
    "histogram": None, "boxplot": None, "dotplot": None,
    "pareto": None, "scatter_plot": None, "time_series_plot": None,
    "contour_plot": None, "simplex_design_plot": None,
}
for cid in GRAPH_IDS:
    add(cid,
        ...
        dataset_hint=GRAPH_DS.get(cid, None),
...
for cid in QT:
    ds = None if cid == "acceptance_sampling" else None
    add(cid,
...
def main() -> None:
    ...
    for e in meta["entries"]:
        ...
        # Wave-5: always prefer v2 lock-table dataset_id; never emit old shared-table ids.
        r["dataset_hint"] = hints.get(cid, "")
