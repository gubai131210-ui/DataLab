#!/usr/bin/env python3
"""Verify all S1-S7 scenario prefilter test names resolve to Qt Creator targets."""
...
def main() -> int:
    ...
    section31 = set(load_section_names())
    scenario_set = set(names)
    ...
    outside = sorted(section31 - scenario_set)
    print(
        f"Scenario prefilter registry: {len(names)} names, "
        f"{len(names) - len(missing)} resolved, {len(missing)} missing"
    )
    if outside:
        print(
            f"INFO: {len(outside)} section 3.1 names not in scenario map "
            "(expected; scenario is S1-S7 acceptance subset, not full 61)"
        )
    for name in missing:
