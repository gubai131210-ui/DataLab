    ...
    std::vector<std::string>& term_names)
{
    ...
    term_names.push_back("X1");
    if (include_interaction) {
        for (std::size_t i = 0; i < q; ++i) {
            term_names.push_back("x" + std::to_string(i + 1) + "*X1");
        }
    }
    ...
    for (std::size_t row = 0; row < components.size(); ++row) {
        ...
        design[row][col++] = process_variable[row];
        if (include_interaction) {
            for (std::size_t i = 0; i < q; ++i) {
                design[row][col++] = components[row][i] * process_variable[row];
            }
        }
    }
    return design;
...
{
    MixtureProcessVariableResult result;
    result.component_order = mixture_model_order_name(options.component_order);
    ...
    if (components.empty() || response.empty() || process_variable.empty()
        || components.size() != response.size()
        || process_variable.size() != response.size()) {
        add_diagnostic(result.diagnostics, DiagnosticMessage::Severity::error,
                       "insufficient_mixture_data", "需要组分、过程变量与响应数据。");
        return result;
    }
    ...
    for (const auto& row : components) {
        ...
        if (std::abs(sum - 1.0) > options.sum_tolerance) {
            add_diagnostic(result.diagnostics, DiagnosticMessage::Severity::warning,
                           ...
                           "分量之和偏离 1（容差 "
                               + std::to_string(options.sum_tolerance) + "）。");
        }
    }

    std::vector<std::string> term_names;
    const Matrix design = build_design(
        components, process_variable, options.component_order,
        options.include_component_process_interaction, term_names);
    const OlsFit fit = fit_ols_no_intercept(design, response);
    if (!fit.ok) {
        add_diagnostic(result.diagnostics, DiagnosticMessage::Severity::error,
                       "ols_failed", "Scheffé OLS 拟合失败。");
        return result;
    }
    if (fit.rank < term_names.size()) {
        add_diagnostic(result.diagnostics, DiagnosticMessage::Severity::warning,
    ...
    if (fit.error_df > 0 && response.size() > fit.rank) {
        result.adjusted_r_squared = 1.0 - (fit.rss / static_cast<double>(fit.error_df))
