...
class TaguchiAnalyzeDialog final : public QDialog {
    Q_OBJECT

public:
    explicit TaguchiAnalyzeDialog(
        const QStringList& column_labels, QWidget* parent = nullptr);

    datalab::domain::TaguchiAnalyzeConfiguration configuration() const;
